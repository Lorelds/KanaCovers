    ...
    <div class="py-12 bg-gray-50">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg sm:rounded-2xl p-8 border border-gray-100">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-6 pb-4 border-b border-gray-100">Venue Information</h3>

                <form method="POST" action="{{ route('admin.venues.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf

                    <div>
                        <label class="block font-medium text-sm text-gray-700 mb-1">Venue Name</label>
                        <input type="text" name="name" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition" placeholder="Ex: Grand City Hall" value="{{ old('name') }}" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <textarea name="address" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition" rows="3" placeholder="Full address">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <button type="submit" class="px-6 py-2.5 bg-black text-white rounded-lg hover:bg-gray-800 shadow-lg transition font-bold">Save Venue</button>
                    </div>
                </form>
